...
export default function FavFilmDetail({ route }) {
	...
	function handleChangeButton() {
		if (!changeButton) {
			...
			setChangeButton(!changeButton);
			return Alert.alert("FAVORITOS", "Añadido de tu lista de favoritos");
		}
	}
	return (
		<ImageBackground
		...
		>
			...
			<View style={styles.buttonsFilm}>
				<TouchableOpacity style={styles.buttonFav} onPress={handleChangeButton}>
					{!changeButton ? (
						<Text style={styles.favText}>Quitar a favoritos</Text>
					) : (
						<Text style={styles.favText}>Añadir de favoritos</Text>
					)}
				</TouchableOpacity>
			</View>
		</ImageBackground>
